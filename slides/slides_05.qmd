...
# Open data - PEGIDA demonstrations {background-color="#ffe0b2"}

::: {style="margin-top: -30px;"}
[common elements of collective identity among PEGIDA participants?]{style="color:indigo;"}
::: 
...
pegpad <- read.csv("../resources/data/pegpad/pegpad_events_final_V1.1.csv", stringsAsFactors = FALSE)
pegpad$event_date <- as.Date(pegpad$event_date)

monthly <- pegpad %>%
  mutate(month = floor_date(event_date, "month")) %>%
  group_by(month, side) %>%
  summarise(n_events = sum(events), .groups = "drop") %>%
  pivot_wider(names_from = side, values_from = n_events, values_fill = 0) %>%
  mutate(total_events = `pro-pegida` + `anti-pegida`) %>%
  arrange(month)

monthly_xts <- xts(
  monthly[, c("total_events", "pro-pegida", "anti-pegida")],
  order.by = monthly$month
)
colnames(monthly_xts) <- c("Total", "Pegida", "Anti-Pegida")

# dygraph(monthly_xts, main = "Pegida and counter-mobilisation events, 2014-2018") %>%
#   dySeries("Total", color = "black", strokeWidth = 2.5) %>%
#   dySeries("Pegida", color = "#b2182b") %>%
#   dySeries("Anti-Pegida", color = "#2166ac") %>%
...
city_coords <- tibble::tribble(
  ...
  "Leverkusen", 51.0459, 6.9847,
  "Ludwigshafen am Rhein", 49.4741, 8.434,
  ...
  "Plauen", 50.4947, 12.1382,
  "Potsdam", 52.3906, 13.0645,
  "Regensburg", 49.0134, 12.1016,
  "Rostock", 54.0887, 12.14,
...
# jitter points slightly so fully overlapping events at the same city remain
# individually visible/clickable, per your request to show each protest as
# its own point rather than aggregating
pegpad_sf <- st_as_sf(pegpad_geo, coords = c("lon", "lat"), crs = 4326) %>%
  st_jitter(amount = 0.1)

# build one mapview layer per year, each with its own matching popup,
# then combine with `+` so they appear as separate, togglable layers.
# (doing it this way avoids a known mapview bug where combining `burst`
# with a pre-built `popup` vector throws a length-mismatch error)
years <- sort(unique(pegpad_sf$year))

year_layers <- lapply(years, function(yr) {
  sub <- pegpad_sf[pegpad_sf$year == yr, ]
  mapview(
    sub,
    zcol = "side",
    ...
    popup = popupTable(
      sub,
      zcol = c("city", "event_date", "side", "events", "participants_mean", "part_violence")
    ),
    layer.name = paste("Pegida events", yr)
  )
})
...
The data is the event-level version of PEGPAD (from Vullers & Hellmeier 2022, "Does counter-mobilization contain right-wing populist movements? Evidence from Germany", *European Journal of Political Research 61*: 21-45). The dygraph plots total monthly events as the filled black line, with the two sides as coloured bars.
...
# Poll: identities and emotions {background-color="#2c844c"}

:::: {.columns}
:::{.column width="35%"}
![](slide_files/05/SMclass5_poll_QR.png){fig-alt="A QR code for the survey."}

:::

::: {.column width="65%" .center}
<!-- go to <https://www.qr-code-generator.com/>, add the link, and screen shot the QR code -->
Take the survey at **<https://forms.gle/TY9iAYvw4KXvuMZMA>**

- which identity is most emphasised?
- which 'out-group' is most often part of the group's narratives

:::
::::

- emotional appeals more important than factual arguments?
- positive emotions more important than negative emotions?
- which emotion seems most central for the movement?
...
surveyResults = liveGoogleSheet(
  "https://docs.google.com/spreadsheets/d/e/" +
    "2PACX-1vQhGEBN4WBNDu4Q5J_sP5aRblp46wM5WgXnG-a7w4ZrfKFPwQeZku_iheBsU6ga2YEdEXnNohZOE_7s/" +
    "pub?gid=1282968597&single=true&output=csv",
...
```{ojs}
// need to replace 'm_identity_emph'

m_identity_emphCounts = aq.from(surveyResults)
  .groupby("m_identity_emph", "movement")
  .count()
  .objects();

plot_m_identity_emph = Plot.plot({
  marks: [
    Plot.frame({
      fill: "#f7f7f7",
      stroke: "black",
    ...
    Plot.barY(m_identity_emphCounts, {
      fx: "m_identity_emph",
      x: "movement",
      y: "count",
      fill: "movement",
...
## Movement responses - out-group

which 'out-group' is most often part of the group's narratives

```{ojs}
// need to replace 'm_outgroup'

m_outgroupCounts = aq.from(surveyResults)
  .groupby("m_outgroup", "movement")
  .count()
  .objects();

plot_m_outgroup = Plot.plot({
  marks: [
    Plot.frame({
      fill: "#f7f7f7",
      stroke: "black",
...
## Movement responses - appeals and positivity 

:::: {.columns}
:::{.column width="50%"}
emotional appeals more important?
<!-- ynm -->

```{ojs}
// need to replace 'm_emotional_appeals'

m_emotional_appealsCounts = aq.from(surveyResults)
  .groupby("m_emotional_appeals", "movement")
  .count()
  .objects();

plot_m_emotional_appeals = Plot.plot({
  marks: [
    Plot.frame({
      fill: "#f7f7f7",
      stroke: "black",
    ...
    Plot.barY(m_emotional_appealsCounts, {
      fx: "m_emotional_appeals",
      x: "movement",
      y: "count",
      fill: "movement",
      stroke: "white"
...
::: {.column width="50%"}
positive emotions more important than negative?
<!-- likert -->

```{ojs}
// need to replace 'm_positive_more_important'

m_positive_more_importantCounts = aq.from(surveyResults)
  .groupby("m_positive_more_important", "movement")
  .count()
  .objects();

plot_m_positive_more_important = Plot.plot({
  marks: [
    Plot.frame({
      fill: "#f7f7f7",
      stroke: "black",
    ...
    Plot.barY(m_positive_more_importantCounts, {
      fx: "m_positive_more_important",
      x: "movement",
      y: "count",
      fill: "movement",
      stroke: "white"
...
## Movement responses - central emotion

which emotion seems most central for the movement?

```{ojs}
// need to replace 'm_central_emotion'

m_central_emotionCounts = aq.from(surveyResults)
  .groupby("m_central_emotion", "movement")
  .count()
  .objects();

plot_m_central_emotion = Plot.plot({
  marks: [
    Plot.frame({
      fill: "#f7f7f7",
      stroke: "black",
    ...
    Plot.barY(m_central_emotionCounts, {
      fx: "m_central_emotion",
      x: "movement",
      y: "count",
      fill: "movement",
      stroke: "white"
...
[Questions about movements next meeting]{style="color:violet;"}: (1) Which organisational form best describes the movement? (2) Does the movement rely on formal coordination structures (meetings, committees, roles)? (3) Does leadership play a visible and strategic role in guiding the movement? (4) What is the movement’s main strategic orientation? (5) Does the movement mainly use conventional, legally permitted forms of protest? (6) What is the movement's primary tactic of mobilisation?
