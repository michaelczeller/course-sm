...
# Poll: counter-mobilisation {background-color="#2c844c"}

:::: {.columns}
:::{.column width="35%"}
![](slide_files/10/SMclass10_poll_QR.png){fig-alt="A QR code for the survey."}

:::

::: {.column width="65%" .center}
<!-- go to <https://www.qr-code-generator.com/>, add the link, and screen shot the QR code -->
Take the survey at **<https://forms.gle/RD2Cb4fEFdufsa8v6>**

- which type of actor provides the strongest opposition to this movement?
- does the movement face sustained organised opposition from another movement(s)?

:::
::::

::: {style="margin-top: -40px;"}
- has counter-mobilisation changed the movement's tactics?
- has counter-mobilisation increased political polarisation around the movement's main issue?
- has counter-mobilisation more strengthened or weakened the movement?
...
surveyResults = liveGoogleSheet(
  "https://docs.google.com/spreadsheets/d/e/" +
    "2PACX-1vT8TUVWJGOEcFN7DahXO1gJT65YBy93ka0sDqH_au2gVPI5d2lbxKFn65vF_t5wVej8iLuovv3eHwrD/" +
    "pub?gid=1419846398&single=true&output=csv",
...
:::: {.columns}
:::{.column width="48%"}
type of actor providing strongest opposition?
<!-- other -->

```{ojs}
// need to replace 'm_actor_opposition_type'

m_actor_opposition_typeCounts = aq.from(surveyResults)
  .groupby("m_actor_opposition_type", "movement")
  .count()
  .objects();

plot_m_actor_opposition_type = Plot.plot({
  marks: [
    Plot.frame({
      fill: "#f7f7f7",
      stroke: "black",
    ...
    Plot.barY(m_actor_opposition_typeCounts, {
      fx: "m_actor_opposition_type",
      x: "movement",
      y: "count",
      fill: "movement",
      stroke: "white"
...
::: {.column width="48%"}
sustained opposition from another movement(s)?
<!-- ynm -->

```{ojs}
// need to replace 'm_sustained_movement_opposition'

m_sustained_movement_oppositionCounts = aq.from(surveyResults)
  .groupby("m_sustained_movement_opposition", "movement")
  .count()
  .objects();

plot_m_sustained_movement_opposition = Plot.plot({
  marks: [
    Plot.frame({
      fill: "#f7f7f7",
      stroke: "black",
    ...
    Plot.barY(m_sustained_movement_oppositionCounts, {
      fx: "m_sustained_movement_opposition",
      x: "movement",
      y: "count",
      fill: "movement",
      stroke: "white"
...
## Movement responses - tactics effect

counter-mobilisation changed the movement's tactics?
<!-- likert -->

```{ojs}
// need to replace 'm_cmob_changed_tactics'

m_cmob_changed_tacticsCounts = aq.from(surveyResults)
  .groupby("m_cmob_changed_tactics", "movement")
  .count()
  .objects();

plot_m_cmob_changed_tactics = Plot.plot({
  marks: [
    Plot.frame({
      fill: "#f7f7f7",
      stroke: "black",
    ...
    Plot.barY(m_cmob_changed_tacticsCounts, {
      fx: "m_cmob_changed_tactics",
      x: "movement",
      y: "count",
      fill: "movement",
      stroke: "white"
...
## Movement responses - issue effect 

counter-mobilisation increased polarisation around the movement's main issue?
<!-- likert -->


```{ojs}
// need to replace 'm_cmob_increased_polarisation'

m_cmob_increased_polarisationCounts = aq.from(surveyResults)
  .groupby("m_cmob_increased_polarisation", "movement")
  .count()
  .objects();

plot_m_cmob_increased_polarisation = Plot.plot({
  marks: [
    Plot.frame({
      fill: "#f7f7f7",
      stroke: "black",
    ...
    Plot.barY(m_cmob_increased_polarisationCounts, {
      fx: "m_cmob_increased_polarisation",
      x: "movement",
      y: "count",
      fill: "movement",
      stroke: "white"
...
## Movement responses - overall effect 

counter-mobilisation more strengthened or weakened the movement?
<!-- other -->

```{ojs}
// need to replace 'm_cmob_overall_effect'

m_cmob_overall_effectCounts = aq.from(surveyResults)
  .groupby("m_cmob_overall_effect", "movement")
  .count()
  .objects();

plot_m_cmob_overall_effect = Plot.plot({
  marks: [
    Plot.frame({
      fill: "#f7f7f7",
      stroke: "black",
    ...
    Plot.barY(m_cmob_overall_effectCounts, {
      fx: "m_cmob_overall_effect",
      x: "movement",
      y: "count",
      fill: "movement",
      stroke: "white"
...
[Questions about movements next meeting]{style="color:violet;"}: (1) Is online activity very important to the movement? (2) What is the most important use of online tools for this movement? (3) Do most new supporters first encounter the movement online? (4) Does online participation frequently lead to offline participation? (5) Which online platform is most important for the movement? (6) Has the movement been affected by social media content moderation policies? (7) Does online participation gives supporters a meaningful sense of belonging, even without attending offline events?
