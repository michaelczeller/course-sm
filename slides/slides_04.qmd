...
::: {.column width="50%" .right}
<div class="tenor-gif-embed" data-postid="25258332" data-share-method="host" data-aspect-ratio="1" data-width="100%"><a href="https://tenor.com/view/the-show-looks-good-lizzo-lizzos-watch-out-for-the-big-grrrls-the-presentation-looks-good-the-performance-looks-good-gif-25258332">The Show Looks Good Lizzo GIF</a>from <a href="https://tenor.com/search/the+show+looks+good-gifs">The Show Looks Good GIFs</a></div> <script type="text/javascript" async src="https://tenor.com/embed.js"></script> 
:::
::::
...
# Open data - suffragette networks {background-color="#ffe0b2"}

::: panel-tabset
### 1908 

```{r suff_net1908, warning=FALSE, message=FALSE}
library(readr)
library(igraph)
library(visNetwork)
library(scales)

raw <- read_csv("../resources/data/suffragettes/50_1908_VISIT_AFF.csv", show_col_types = FALSE)
names(raw)[1] <- "person"

mat <- as.matrix(raw[, -1])
rownames(mat) <- raw$person
diag(mat) <- 0
...
V(g)$degree <- degree(g)
V(g)$size   <- rescale(V(g)$degree, to = c(15, 45))
V(g)$color  <- ifelse(V(g)$name == "Blathwayt Family", "gold", "steelblue")
V(g)$title  <- paste0(V(g)$name, " (", V(g)$degree, " connections)")
...
visIgraph(g, idToLabel = TRUE) %>%
  ...
  visNodes(font = list(size = 16), borderWidth = 1,
           color = list(border = "white", highlight = "red")) %>%
  visEdges(color = list(color = "lightgrey", highlight = "red"), smooth = FALSE) %>%
  visOptions(highlightNearest = list(enabled = TRUE, degree = 1, hover = TRUE),
             nodesIdSelection = TRUE) %>%
  visLegend(main = "1908 co-visitation network", width = 0.15)
```

### 1909 

```{r suff_net1909, warning=FALSE, message=FALSE}
library(readr)
library(igraph)
library(visNetwork)
library(scales)

raw <- read_csv("../resources/data/suffragettes/50_1909_VISIT_AFF.csv", show_col_types = FALSE)
names(raw)[1] <- "person"
...
visIgraph(g, idToLabel = TRUE) %>%
  ...
  visOptions(highlightNearest = list(enabled = TRUE, degree = 1, hover = TRUE),
             nodesIdSelection = TRUE) %>%
  visLegend(main = "1909 co-visitation network", width = 0.15)
```

### 1910 

```{r suff_net1910, warning=FALSE, message=FALSE}
library(readr)
library(igraph)
library(visNetwork)
library(scales)

raw <- read_csv("../resources/data/suffragettes/50_1910_VISIT_AFF.csv", show_col_types = FALSE)
names(raw)[1] <- "person"
...
visIgraph(g, idToLabel = TRUE) %>%
  ...
  visOptions(highlightNearest = list(enabled = TRUE, degree = 1, hover = TRUE),
             nodesIdSelection = TRUE) %>%
  visLegend(main = "1910 co-visitation network", width = 0.15)
```

### 1911 

```{r suff_net1911, warning=FALSE, message=FALSE}
library(readr)
library(igraph)
library(visNetwork)
library(scales)

raw <- read_csv("../resources/data/suffragettes/50_1911_VISIT_AFF.csv", show_col_types = FALSE)
names(raw)[1] <- "person"
...
visIgraph(g, idToLabel = TRUE) %>%
  ...
  visOptions(highlightNearest = list(enabled = TRUE, degree = 1, hover = TRUE),
             nodesIdSelection = TRUE) %>%
  visLegend(main = "1911 co-visitation network", width = 0.15)
```

### 1912 

```{r suff_net1912, warning=FALSE, message=FALSE}
library(readr)
library(igraph)
library(visNetwork)
library(scales)

raw <- read_csv("../resources/data/suffragettes/50_1912_VISIT_AFF.csv", show_col_types = FALSE)
names(raw)[1] <- "person"
...
visIgraph(g, idToLabel = TRUE) %>%
  ...
  visOptions(highlightNearest = list(enabled = TRUE, degree = 1, hover = TRUE),
             nodesIdSelection = TRUE) %>%
  visLegend(main = "1912 co-visitation network", width = 0.15)
```

### 1913 

```{r suff_net1913, warning=FALSE, message=FALSE}
library(readr)
library(igraph)
library(visNetwork)
library(scales)

raw <- read_csv("../resources/data/suffragettes/50_1913_VISIT_AFF.csv", show_col_types = FALSE)
names(raw)[1] <- "person"
...
visIgraph(g, idToLabel = TRUE) %>%
  ...
  visOptions(highlightNearest = list(enabled = TRUE, degree = 1, hover = TRUE),
             nodesIdSelection = TRUE) %>%
  visLegend(main = "1913 co-visitation network", width = 0.15)
```


### Explanatory note

Data source: Blathwayt "Eagle House" garden visitors' book, Bristol/Bath area; Manchester Mitchell Centre for Social Network Analysis, Suffragettes dataset: <https://sites.google.com/site/ucinetsoftware/datasets/covert-networks/suffragettes>. Collected by Gemma Edwards based on archival material of about UK suffragettes. 

::: 

## COULD ADD VIS OF NETWORK OVER TIME {visibility="hidden"}
...
- social media networks have eased mobilisation?
- are offline, 'brick and mortar' orgs as important as ever?
- young people more prominent, numerous in activism?
- is it advantageous to have more young people in activism?
...
[[Questions about movements]{style="color:violet;"}: (1) Does joining this movement require high level of commitment? (2) Does the movement rely heavily on social media? (3) Which setting plays the biggest role in mobilising support? (4) How are most participants recruited? (5) Which resource seems most important for this movement's success?]{style="font-size: 0.8em;"}
:::
...
## Movement responses - commitment

joining requires high commitment

```{ojs}
// need to replace 'm_high_commitment'

m_high_commitmentCounts = aq.from(surveyResults)
  .groupby("m_high_commitment", "movement")
  .count()
  .objects();

plot_m_high_commitment = Plot.plot({
  marks: [
    Plot.frame({
      fill: "#f7f7f7",
      stroke: "black",
    ...
    Plot.barY(m_high_commitmentCounts, {
      fx: "m_high_commitment",
      x: "movement",
      y: "count",
      fill: "movement",
      stroke: "white"
...
## Movement responses - joining

:::: {.columns}
:::{.column width="48%"}
Key setting for mobilising support?

```{ojs}
// need to replace 'm_mobilisation_setting'

m_mobilisation_settingCounts = aq.from(surveyResults)
  .groupby("m_mobilisation_setting", "movement")
  .count()
  .objects();

plot_m_mobilisation_setting = Plot.plot({
  marks: [
    Plot.frame({
      fill: "#f7f7f7",
      stroke: "black",
    ...
    Plot.barY(m_mobilisation_settingCounts, {
      fx: "m_mobilisation_setting",
      x: "movement",
      y: "count",
      fill: "movement",
      stroke: "white"
...
::: {.column width="48%"}
Recruitment most likely through...

```{ojs}
// need to replace 'm_recruitment_through'

m_recruitment_throughCounts = aq.from(surveyResults)
  .groupby("m_recruitment_through", "movement")
  .count()
  .objects();

plot_m_recruitment_through = Plot.plot({
  marks: [
    Plot.frame({
      fill: "#f7f7f7",
      stroke: "black",
    ...
    Plot.barY(m_recruitment_throughCounts, {
      fx: "m_recruitment_through",
      x: "movement",
      y: "count",
      fill: "movement",
      stroke: "white"
...
## Movement responses - social media

movement relies heavily on social media

```{ojs}
// need to replace 'm_social_media_reliance'

m_social_media_relianceCounts = aq.from(surveyResults)
  .groupby("m_social_media_reliance", "movement")
  .count()
  .objects();

plot_m_social_media_reliance = Plot.plot({
  marks: [
    Plot.frame({
      fill: "#f7f7f7",
      stroke: "black",
    ...
    Plot.barY(m_social_media_relianceCounts, {
      fx: "m_social_media_reliance",
      x: "movement",
      y: "count",
      fill: "movement",
      stroke: "white"
...
## Movement responses - key resource

movement's most importance resource for success?


```{ojs}
// need to replace 'm_most_important_resource'

m_most_important_resourceCounts = aq.from(surveyResults)
  .groupby("m_most_important_resource", "movement")
  .count()
  .objects();

plot_m_most_important_resource = Plot.plot({
  marks: [
    Plot.frame({
      fill: "#f7f7f7",
      stroke: "black",
    ...
    Plot.barY(m_most_important_resourceCounts, {
      fx: "m_most_important_resource",
      x: "movement",
      y: "count",
      fill: "movement",
      stroke: "white"
...
[Questions about movements next meeting]{style="color:violet;"}: (1) Which identity is most strongly emphasised? (2) Which "out-group" is most frequently part of the group's narratives? (3) Are emotional appeals more important than factual or technical arguments? (4) Are positive emotions more important than negative emotions in sustaining participation? (5) Which emotion seems most central to the movement's mobilisation?
