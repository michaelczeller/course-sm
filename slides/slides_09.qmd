...
- why is a movement treated as 'threatening'?
- what happens after state represses a movement?
- states harsher to 'marginalised group' movements? 
...
[[Questions about movements]{style="color:violet;"}: (1) how has state responded to the movement? (2) state responses focus more on movement's tactics than on its demands? (3) different state institutions respond differently to the movement? (4) state's response changed over time? (5) are institutional channels available to the movement? (6) participants often face coercive state responses? (7) movement adapted its tactics after policing or legal actions? (8) state has strengthened or weakened the movement?]{style="font-size: 0.7em;"}
:::
...
surveyResults = liveGoogleSheet(
  ...
  10000, 1, 17); // adjust the last number to select all relevant columns 
...
## Movement responses - state responses 

state responses have mostly been...
<!-- other -->

```{ojs}
// need to replace 'm_state_responses_generally'

m_state_responses_generallyCounts = aq.from(surveyResults)
  .groupby("m_state_responses_generally", "movement")
  .count()
  .objects();

plot_m_state_responses_generally = Plot.plot({
  marks: [
    Plot.frame({
      fill: "#f7f7f7",
      stroke: "black",
    ...
    Plot.barY(m_state_responses_generallyCounts, {
      fx: "m_state_responses_generally",
      x: "movement",
      y: "count",
      fill: "movement",
      stroke: "white"
...
state responses focus more on tactics than demands 
<!-- likert -->

```{ojs}
// need to replace 'm_state_focuses_on_tactics'

m_state_focuses_on_tacticsCounts = aq.from(surveyResults)
  .groupby("m_state_focuses_on_tactics", "movement")
  .count()
  .objects();

plot_m_state_focuses_on_tactics = Plot.plot({
  marks: [
    Plot.frame({
      fill: "#f7f7f7",
      stroke: "black",
    ...
    Plot.barY(m_state_focuses_on_tacticsCounts, {
      fx: "m_state_focuses_on_tactics",
      x: "movement",
      y: "count",
      fill: "movement",
      stroke: "white"
...
do different state institutions respond differently
<!-- ynm -->

```{ojs}
// need to replace 'm_diff_state_diff_response'

m_diff_state_diff_responseCounts = aq.from(surveyResults)
  .groupby("m_diff_state_diff_response", "movement")
  .count()
  .objects();

plot_m_diff_state_diff_response = Plot.plot({
  marks: [
    Plot.frame({
      fill: "#f7f7f7",
      stroke: "black",
    ...
    Plot.barY(m_diff_state_diff_responseCounts, {
      fx: "m_diff_state_diff_response",
      x: "movement",
      y: "count",
      fill: "movement",
      stroke: "white"
...
state response changed over time
<!-- likert -->

```{ojs}
// need to replace 'm_state_responses_over_time'

m_state_responses_over_timeCounts = aq.from(surveyResults)
  .groupby("m_state_responses_over_time", "movement")
  .count()
  .objects();

plot_m_state_responses_over_time = Plot.plot({
  marks: [
    Plot.frame({
      fill: "#f7f7f7",
      stroke: "black",
    ...
    Plot.barY(m_state_responses_over_timeCounts, {
      fx: "m_state_responses_over_time",
      x: "movement",
      y: "count",
      fill: "movement",
      stroke: "white"
...
## Movement responses - institutional openness

state provides open institutional channels
<!-- likert -->

```{ojs}
// need to replace 'm_state_provides_channels'

m_state_provides_channelsCounts = aq.from(surveyResults)
  .groupby("m_state_provides_channels", "movement")
  .count()
  .objects();

plot_m_state_provides_channels = Plot.plot({
  marks: [
    Plot.frame({
      fill: "#f7f7f7",
      stroke: "black",
    ...
    Plot.barY(m_state_provides_channelsCounts, {
      fx: "m_state_provides_channels",
      x: "movement",
      y: "count",
      fill: "movement",
      stroke: "white"
...
## Movement responses - coercion 

frequency of coercive state responses
<!-- other -->

```{ojs}
// need to replace 'm_frequent_coercion'

m_frequent_coercionCounts = aq.from(surveyResults)
  .groupby("m_frequent_coercion", "movement")
  .count()
  .objects();

plot_m_frequent_coercion = Plot.plot({
  marks: [
    Plot.frame({
      fill: "#f7f7f7",
      stroke: "black",
    ...
    Plot.barY(m_frequent_coercionCounts, {
      fx: "m_frequent_coercion",
      x: "movement",
      y: "count",
      fill: "movement",
      stroke: "white"
...
## Movement responses - movement adaptation 

movement adapted its tactics after police/legal actions
<!-- ynm -->

```{ojs}
// need to replace 'm_adapted_tactics'

m_adapted_tacticsCounts = aq.from(surveyResults)
  .groupby("m_adapted_tactics", "movement")
  .count()
  .objects();

plot_m_adapted_tactics = Plot.plot({
  marks: [
    Plot.frame({
      fill: "#f7f7f7",
      stroke: "black",
    ...
    Plot.barY(m_adapted_tacticsCounts, {
      fx: "m_adapted_tactics",
      x: "movement",
      y: "count",
      fill: "movement",
      stroke: "white"
...
## Movement responses - state effect 

state responses strengthened or weakened movement
<!-- other -->

```{ojs}
// need to replace 'm_strengthened_or_weakened'

m_strengthened_or_weakenedCounts = aq.from(surveyResults)
  .groupby("m_strengthened_or_weakened", "movement")
  .count()
  .objects();

plot_m_strengthened_or_weakened = Plot.plot({
  marks: [
    Plot.frame({
      fill: "#f7f7f7",
      stroke: "black",
    ...
    Plot.barY(m_strengthened_or_weakenedCounts, {
      fx: "m_strengthened_or_weakened",
      x: "movement",
      y: "count",
      fill: "movement",
      stroke: "white"
...
# acled <- fread("../resources/data/acled/ACLEDdata.csv",
#                select = c("event_date", "interaction", "region", "country",
#                           "location", "latitude", "longitude", "fatalities", "notes"))
# acled[, event_date := as.Date(event_date)]
# 
# state_protester <- acled[interaction == "State forces-Protesters" &
#                             region == "Europe" &
#                             event_date >= as.Date("2022-01-01")]
# state_protester[, year := format(event_date, "%Y")]
# 
# write.csv(state_protester, file = "acled_state_protester.csv")

acled <- fread("../resources/data/acled/acled_state_protester.csv",)
...
state_sf <- st_as_sf(state_protester, coords = c("longitude", "latitude"), crs = 4326)

mapview(
  state_sf,
  zcol = "year",
  layer.name = "State forces vs. protesters",
  popup = popupTable(
    state_sf,
    zcol = c("event_date", "country", "location", "fatalities", "notes")
  )
)

# to widen the scope (slower to render), e.g. all years and/or all regions:
# state_protester <- acled[interaction == "State forces-Protesters" & region == "Europe"]
# state_protester <- acled[interaction == "State forces-Protesters"]
...
Data source: ACLED (Armed Conflict Location & Event Data Project, <https://acleddata.com/>). Each point is an event where ACLED's `interaction` variable is coded `"State forces-Protesters"` (i.e. direct engagement between state forces and protesters, from crowd-control interventions to violent crackdowns). This is limited to `region == "Europe"` and 2022 onwards (\~3,000 points). Clicking a point opens a popup table including the `notes` field, so students can read what actually happened at a given event.
...
state_protester %>% download_this(
    output_name = "ACLED_state_protester",
...
[Questions about movements next meeting]{style="color:violet;"}: (1) Which type of actor provides the strongest opposition to this movement? (2) Does the movement face sustained organised opposition from another movement(s)? (3) Has counter-mobilisation changed the movement's tactics? (4) Has counter-mobilisation increased political polarisation around the movement's main issue? (5) Has counter-mobilisation more strengthened or weakened the movement?
