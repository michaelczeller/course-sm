...
- short-term coalitions more likely to achieve goals?
- factor most important in coalition formation?
- biggest benefit in forming a coalition?
- identity-based movements struggle to coalition?
...
[[Questions about movements]{style="color:violet;"}: (1) coalition is important for this movement's success? (2) Has the movement participated in broad “umbrella coalitions” that bring together diverse actors? (3) Are coalition partners seen as just temporary allies? (4) Do coalition actions tend to be more effective than solo actions by the movement? (5) Are the movement’s coalitions are primarily local rather than national or international? (6) Does the movement have supportive allies in one or more political party?]{style="font-size: 0.8em;"}
...
## Movement responses - joining coalitions 

:::: {.columns}
:::{.column width="48%"}

Coalition important for this movement's success
<!-- likert -->

```{ojs}
// need to replace 'm_coalition_important'

m_coalition_importantCounts = aq.from(surveyResults)
  .groupby("m_coalition_important", "movement")
  .count()
  .objects();

plot_m_coalition_important = Plot.plot({
  marks: [
    Plot.frame({
      fill: "#f7f7f7",
      stroke: "black",
    ...
    Plot.barY(m_coalition_importantCounts, {
      fx: "m_coalition_important",
      x: "movement",
      y: "count",
      fill: "movement",
      stroke: "white"
...
Movement has participated in 'umbrella coalitions'
<!-- ynm -->

```{ojs}
// need to replace 'm_umbrella_coalitions'

m_umbrella_coalitionsCounts = aq.from(surveyResults)
  .groupby("m_umbrella_coalitions", "movement")
  .count()
  .objects();

plot_m_umbrella_coalitions = Plot.plot({
  marks: [
    Plot.frame({
      fill: "#f7f7f7",
      stroke: "black",
    ...
    Plot.barY(m_umbrella_coalitionsCounts, {
      fx: "m_umbrella_coalitions",
      x: "movement",
      y: "count",
      fill: "movement",
      stroke: "white"
...
## Movement responses - partners

:::: {.columns}
:::{.column width="48%"}

coalition partners seen as temporary allies
<!-- ynm -->

```{ojs}
// need to replace 'm_coalition_partners'

m_coalition_partnersCounts = aq.from(surveyResults)
  .groupby("m_coalition_partners", "movement")
  .count()
  .objects();

plot_m_coalition_partners = Plot.plot({
  marks: [
    Plot.frame({
      fill: "#f7f7f7",
      stroke: "black",
    ...
    Plot.barY(m_coalition_partnersCounts, {
      fx: "m_coalition_partners",
      x: "movement",
      y: "count",
      fill: "movement",
      stroke: "white"
...
movement's coalitions are primarily local
<!-- likert -->

```{ojs}
// need to replace 'm_coalition_local'

m_coalition_localCounts = aq.from(surveyResults)
  .groupby("m_coalition_local", "movement")
  .count()
  .objects();

plot_m_coalition_local = Plot.plot({
  marks: [
    Plot.frame({
      fill: "#f7f7f7",
      stroke: "black",
    ...
    Plot.barY(m_coalition_localCounts, {
      fx: "m_coalition_local",
      x: "movement",
      y: "count",
      fill: "movement",
      stroke: "white"
...
## Movement responses - coalition efficacy

coalition actions tend to be more effective
<!-- ynm -->

```{ojs}
// need to replace 'm_coalition_action_effective'

m_coalition_action_effectiveCounts = aq.from(surveyResults)
  .groupby("m_coalition_action_effective", "movement")
  .count()
  .objects();

plot_m_coalition_action_effective = Plot.plot({
  marks: [
    Plot.frame({
      fill: "#f7f7f7",
      stroke: "black",
    ...
    Plot.barY(m_coalition_action_effectiveCounts, {
      fx: "m_coalition_action_effective",
      x: "movement",
      y: "count",
      fill: "movement",
      stroke: "white"
...
## Movement responses - party allies 

Movement has political party allies
<!-- ynm -->

```{ojs}
// need to replace 'm_party_allies'

m_party_alliesCounts = aq.from(surveyResults)
  .groupby("m_party_allies", "movement")
  .count()
  .objects();

plot_m_party_allies = Plot.plot({
  marks: [
    Plot.frame({
      fill: "#f7f7f7",
      stroke: "black",
    ...
    Plot.barY(m_party_alliesCounts, {
      fx: "m_party_allies",
      x: "movement",
      y: "count",
      fill: "movement",
...
party_labels <- c(
  actor_party_left    = "Left party",
  actor_party_right   = "Right party",
  actor_party_unknown = "Unknown/unclear party"
)

party_yearly <- poldem %>%
  select(year, all_of(names(party_labels))) %>%
  pivot_longer(-year, names_to = "actor", values_to = "flag") %>%
  filter(flag == "yes") %>%
  mutate(actor = recode(actor, !!!party_labels)) %>%
  count(year, actor) %>%
  mutate(tooltip = paste0(actor, ", ", year, ": ", n, " events"))

p <- ggplot(party_yearly, aes(x = year, y = n, fill = actor,
                               tooltip = tooltip, data_id = actor)) +
  geom_col_interactive(position = position_dodge(width = 0.8), width = 0.7) +
  scale_x_continuous(breaks = seq(2000, 2015, 1)) +
  labs(x = NULL, y = "No. of protest events",
       fill = "Party actor involved",
       title = "Protest events with party involvement, by orientation",
       subtitle = "30 European countries, 2000-2015 (PolDem)") +
  theme_minimal(base_size = 13) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))

girafe(
  ggobj = p, width_svg = 10, height_svg = 5.5,
  options = list(
    opts_hover(css = "stroke:black;stroke-width:1.5px;"),
    opts_hover_inv(css = "opacity:0.3;"),
    opts_tooltip(css = "background-color:white;padding:4px;border-radius:3px;")
  )
)
...
Data source: Kriesi et al. 2020, [PolDem-Protest Dataset](https://poldem.eui.eu/) 30 European Countries, Version 1.
...
party_yearly <- party_yearly %>% select(year, actor, n)

reactable::reactable(party_yearly,
  defaultPageSize = 5,
...
party_yearly %>% download_this(
    output_name = "PolDem_party_yearly",
...
[Questions about movements next meeting]{style="color:violet;"}: (1) How has the state generally responded? (2) Do state responses focus more on the movement's tactics than on its political demands? (3) Do different state institutions respond differently? (4) Has the state's response changed significantly over time? (5) Do state authorities provide institutional channels through which the movement can pursue its goals? (6) Do participants often face coercive state responses? (7) Has the movement adapted its tactics in response to policing or legal actions? (8) Have state responses more strengthened or weakened the movement?
